...
export default function PostsCatalogClient({ initialPosts = [] }) {
  ...
  const toggleLike = async (postId) => {
    ...
    try {
      ...
      setRemotePosts((prev) => prev.map((post) => post.id === postId
        ? { ...post, isLikedByUser: liked, initialLikes: Math.max(0, (post.initialLikes || 0) + (liked ? 1 : -1)) }
        : post
      ));
    } catch (error) {
    ...
    <div className="posts-page-wrapper">
        ...
        <section className="posts-my-section">
          <div className="container">
            <div className="posts-my-articles posts-my-articles-top">
              <div className="posts-my-articles-heading">
                <h2>{t("postsPage.myArticles")}</h2>
                <span>{remotePosts.filter((post) => post.authorId === user.uid).length}</span>
              </div>
              {remotePosts.filter((post) => post.authorId === user.uid).length === 0 ? (
                <p>{t("postsPage.noArticlesYet")}</p>
              ) : (
                <div className="posts-my-articles-grid">
                  {remotePosts.filter((post) => post.authorId === user.uid).map((post) => (
                    <article key={post.id} className="posts-my-article">
                      ...
                      <div>
                        <button type="button" onClick={() => startEditPost(post)}>{t("postsPage.edit")}</button>
                        <button type="button" onClick={() => setDeletePostTarget(post)}>{t("postsPage.delete")}</button>
                      </div>
                    </article>
                  ))}
                </div>
      ...
      <section className="section posts-feed-section">
        <div className="container posts-feed-container posts-layout">
          ...
          <div className="posts-main-feed">
            <form className="posts-composer" onSubmit={submitPost}>
              <div className="posts-composer-top">
                {user?.photoURL ? (
                  <img src={user.photoURL} alt="" className="posts-composer-avatar" />
                ) : (
                  <BrandLogo width={42} height={42} />
                )}
                <span>
                  {user
                    ? (user.displayName || user.email?.split("@")[0] || t("postsPage.userDefault"))
                    : t("postsPage.loginToWrite")}
                </span>
              </div>
              <textarea
                value={composerText}
                onChange={(e) => setComposerText(e.target.value)}
                placeholder={t("postsPage.textareaPlaceholder")}
                disabled={!user}
              />
              <div className="posts-photo-row">
                <label className="posts-photo-upload">
                  <input
                    type="file"
                    accept="image/*"
                    disabled={!user || uploadingPostImage}
                    onChange={(e) => uploadPostImage(e.target.files?.[0])}
                  />
                ...
                {postImage && (
                  <div className="posts-photo-preview">
                    ...
                    <button type="button" onClick={() => setPostImage("")}>×</button>
                  </div>
                )}
              </div>
              ...
              {composerMode === "poll" && (
                <div className="posts-composer-extra">
                  ...
                  <input
                    ...
                    placeholder={t("postsPage.pollQuestionPlaceholder")}
                  />
                  {pollOptions.map((option, index) => (
                    <input
                      ...
                      placeholder={
                        (lang === "en" ? "Option " : lang === "ru" ? "Вариант " : lang === "tr" ? "Seçenek " : lang === "ar" ? "خيار " : "პასუხი ") +
                        (index + 1)
                      }
                    />
                  ))}
                  <button
                    type="button"
                    className="posts-add-poll-option"
                    onClick={() => setPollOptions((prev) => [...prev, ""])}
                  >
                    {t("postsPage.addOption")}
                  </button>
                </div>
              )}
              <div className="posts-composer-actions">
                <button
                  type="button"
                  onClick={() => setComposerMode(composerMode === "feeling" ? null : "feeling")}
                >
                  {t("postsPage.feelingLabel")}
                </button>
                <button
                  type="button"
                  onClick={() => setComposerMode(composerMode === "poll" ? null : "poll")}
                >
                  {t("postsPage.pollLabel")}
                </button>
                {user && (
            ...
            {loading ? (
              ...
              <div className="posts-empty-state">
                <h2>{t("postsPage.noPostsYet")}</h2>
                <p>{t("postsPage.noPostsDesc")}</p>
              </div>
            ) : (
              postsData.map((post) => {
                const isLiked = !!post.isLikedByUser;
                const currentLikes = post.initialLikes || 0;
                  ...
                  <article key={post.id} className="facebook-post-card">
                      ...
                      <div className="fb-author-wrap">
                        ...
                        <div className="fb-author-info">
                          <div className="fb-name-row">
                            ...
                            {post.verified && (
                              <span className="fb-verified-badge" title={t("postsPage.verifiedPage")}>✓</span>
                            )}
                          </div>
                    ...
                    <div className="fb-reactions-bar">
                      ...
                      <div className="fb-counts-group">
                        ...
                        <span className="fb-dot">•</span>
                        <span className="fb-count-item">{post.sharesCount || 0} {t("postsPage.sharesCount")}</span>
                      </div>
                    </div>
