...
export default function HomePageClient({
  ...
  const popularPlaces = useMemo(() => {
    const explicitlyPopular = places.filter((place) => place.isPopular);
    return (explicitlyPopular.length > 0 ? explicitlyPopular : places).slice(0, 2);
  }, [places]);
  ...
  const popularTours = useMemo(() => {
    const explicitlyPopular = allTours.filter((tour) => tour.isPopular);
    return explicitlyPopular.length > 0 ? explicitlyPopular : allTours.slice(0, 6);
  }, [allTours]);
  ...
  const dynamicSections = useMemo(() => {
    ...
    allTours.forEach((tour) => {
      const secKey = tour.tourSection || tour.category;
