...
export default function PlaceDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    Promise.all([getPlace(id), listPlaces()])
      .then(([item, items]) => { setPlace(item); setAll(items); })
  ...
  if (loading) return <><Navbar active="places" /><main className="place-detail-state">{t("placesPage.loading")}</main><Footer /></>;
  if (!place) return <><Navbar active="places" /><main className="place-detail-state"><h1>{t("placeDetail.notFoundTitle")}</h1><Link href="/places">{t("placeDetail.notFoundBtn")}</Link></main><Footer /></>;
