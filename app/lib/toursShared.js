// Pure tour helpers (no Firebase). Safe to import from client components
// without pulling the Firebase SDK into the page bundle.
  ...
  const itinerary = (Array.isArray(tour.itinerary) ? tour.itinerary : []).map((item) => ({
    ...
    placeSlug:
      typeof item?.placeId === "string" && item.placeId
        ? getContentSlug(customPlaces.find((p) => p?.id === item.placeId)) || item.placeId
        : "",
...
// URL slug helpers (see slugs.js for the lookup side).
