...
(function () {
  ...
  const TRANSLATABLE_FIELDS = {
    ...
    post: [
      ...
      { key: 'readTime', inputId: 'post-readtime', label: 'Read Time',type: 'input' },
    ],
    featured: [
  ...
  async function generate(entity) {
    const statusEl = document.getElementById(`${entity}-translate-status`);
    const btn = document.getElementById(`${entity}-translate-btn`);
      ...
      for (const field of fields) {
        ...
        if (field.type === 'array' || field.type === 'array-csv') {
          ...
          for (let i = 0; i < src.length; i++) {
            ...
            const obj = await window.GTTranslate.translateToAllLangs(src[i]);
          ...
          if (!s) continue;
          // eslint-disable-next-line no-await-in-loop
          const obj = await window.GTTranslate.translateToAllLangs(s);
          const langs = {};
